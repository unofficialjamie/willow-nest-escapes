...
import { Wifi, Car, Coffee, Waves, Dumbbell, Shield, Users, Utensils, MapPin, Star, Award, Heart, LucideIcon } from "lucide-react";
...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Clean up widget divs
      const widgetDiv = document.getElementById('quickbook-widget');
      if (widgetDiv) widgetDiv.innerHTML = '';
      const widgetDivMobile = document.getElementById('quickbook-widget-mobile');
      if (widgetDivMobile) widgetDivMobile.innerHTML = '';
    };
  }, []);
  ...
  return <>
      ...
      <section className="relative min-h-screen flex flex-col justify-between text-white hero-gradient overflow-hidden">
        ...
        <div className="container mx-auto px-4 pb-8">
          <div id="quickbook-widget" className="min-h-[100px] max-w-[1000px] mx-auto hidden md:block"></div>
        </div>
      </section>
      ...
      <div className="block md:hidden bg-background py-6">
        <div className="container mx-auto px-4">
          <div id="quickbook-widget-mobile" className="min-h-[100px] max-w-[1000px] mx-auto"></div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {highlightsData.items?.map((highlight: any, index: number) => {
              ...
              return <Card key={index} className="card-luxury p-6 text-center hover-lift h-full flex flex-col">
                  <div className="mb-4 flex justify-center">
                    {isImageUrl ? <img src={highlight.icon} alt="" className="w-8 h-8 sm:w-12 sm:h-12 object-contain" /> : IconComponent ? <IconComponent className="h-8 w-8 sm:h-12 sm:w-12 text-primary" /> : <MapPin className="h-8 w-8 sm:h-12 sm:w-12 text-primary" />}
                  </div>
                  ...
                  <div className="relative h-80">
                    ...
                    <div className="absolute bottom-6 left-6 right-6 text-white">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {facility.features.map((feature: string, idx: number) => <div key={idx} className="flex items-center gap-2 text-sm">
      ...
      <section className="py-16 bg-luxury-cream">
                  ...
                  <div className="space-y-4">
                    ...
                    return <div key={index} className="flex items-start gap-3">
                          <div className="w-6 h-6 sm:w-8 sm:h-8 bg-primary/20 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                            {isImageUrl ? <img src={value.icon} alt="" className="w-3 h-3 sm:w-4 sm:h-4 object-contain" /> : IconComponent ? <IconComponent className="h-3 w-3 sm:h-4 sm:w-4 text-primary" /> : <Star className="h-3 w-3 sm:h-4 sm:w-4 text-primary" />}
                          </div>
                          <div>
        ...
        <section className="py-16 bg-luxury-cream">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {testimonialsData.items?.map((testimonial: any, index: number) => <Card key={index} className="card-luxury">
                  <CardContent className="p-6">
                    <div className="flex items-center mb-4">
                      {[...Array(testimonial.rating)].map((_, i) => <Star key={i} className="h-5 w-5 text-primary fill-primary" />)}
                    </div>
        ...
        <section className="py-16 bg-background">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              return <Card key={index} className="card-luxury p-6 text-center hover-lift h-full flex flex-col">
                    <div className="mb-4 flex justify-center">
                      {isImageUrl ? <img src={amenity.icon} alt="" className="w-8 h-8 sm:w-12 sm:h-12 object-contain" /> : IconComponent ? <IconComponent className="h-8 w-8 sm:h-12 sm:w-12 text-primary" /> : <Wifi className="h-8 w-8 sm:h-12 sm:w-12 text-primary" />}
                    </div>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                {offersData.packages?.map((pkg: any, index: number) => <Card key={index} className="bg-background/95 backdrop-blur-sm">
                    <CardContent className="p-6 text-left">
                      ...
                      <ul className="text-sm text-muted-foreground space-y-1 mb-4">
                        {pkg.features.map((feature: string, idx: number) => <li key={idx}>• {feature}</li>)}
                      </ul>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-2xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                <input type="email" placeholder="Enter your email address" className="flex-1 px-4 py-3 rounded-md border-0 text-foreground" />
